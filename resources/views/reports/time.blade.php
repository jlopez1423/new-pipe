...
@section('content')
    <div id="container">
        <canvas class="graph" id="app" width="400" height="400">

        </canvas>
        <button id="addDataset">ADD</button>
    </div>
@endsection
...
@section('footer')
    ...
    $(document).ready(function(){
        $.getJSON('/api/reports/time',{},function( response ){
            ...
            response.clients.forEach(function(task){
              taskInfo.push(task);
              taskHours.push(task.total_time);
              taskName.push(task.name);
              taskColor.push(getRandomColor());
              task.projects.forEach(function(project, index){
                projectTimes[index] = project;
              });
              projectsArray.push(projectTimes);
              projectTimes = [];
            });
            ...
            projectsArray.forEach(function(projects, index){
              var array = {
                data: [],
                backgroundColor: [],
                label: [],
              ...
              projects.forEach(function(project, index){
                tempObject.data = project.total_time;
                tempObject.backgroundColor = getRandomColor();
                array['data'].push((project.total_time));
                array['backgroundColor'].push(getRandomColor());
                array['label'].push(project.name);
              });
              projectObjects.push(array);
            ...
            document.getElementById('addDataset').addEventListener('click', function() {
                ...
                data.datasets.push(projectObjects[i]);
                i++;
                myPieChart.update();
            });
